...
from place.views import get_coordinates
...
def view_orders(request):
    ...
    orders_coordinates = get_coordinates([order.address for order in orders])
    restaurants_distance = {}
    for order in orders:
        order_coordinates = orders_coordinates.get(order.address)
        restaurants_coordinates = get_coordinates([address for address in Restaurant.objects.values_list('address', flat=True)])
        for restaurant in order.restaurants:
            if order_coordinates:
                restaurant_distance = round(distance.distance(order_coordinates, restaurants_coordinates.get(restaurant.address)).km, 3)
